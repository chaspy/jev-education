> 2026-09-20: このアプリは旧実験の履歴です。公開URLのトップは、`apps/lesson-fit`の新しい直接選択実験に置き換わりました。旧版は `/previous/index.html` で参照できます。`explorer:dev/deploy` は新版を起動・公開します。
...
- 仮説→方法→答え合わせの作り方→具体例→結果→分析→示唆の順に説明。
- 分母をなくす教材を探す1件を、6ステップで操作しながら追う。
- 代表的な改善・検索漏れ・該当なし・ラベル問題・API不要の例をワンクリックで選択。
- 全34ケースを選べる。架空の先生について作成した入力と、検索→条件処理→Jev→閾値の各段階を表示。
- 教材本文・模範解答・条件違反・採点用ラベル・元のAPI request/responseを確認。
- 閾値スライダーで保存済みのNoul値を再集計。実験の0.65と変更後を明示。新しい推論や課金は発生しない。
- 全指標の母数、元データの限界、こちらが作った採点表の粗さ、時間と費用を説明。

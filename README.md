...
手動確認：かっこを含む方程式の答案Aをクリック → 分配法則の確認問題で誤答 →
復習教材 → 宿題セットを保存 → 元の問題に再挑戦して `x = 3`。
同じ問題の答案Bでは、異なる戻り先になることを確認できます。
...
[測定方法・結果・再実行手順](benchmarks/README.md)を公開しています。
MaE公開データ151件の55分類ではTop-1 58.3%、Top-3 79.5%。
保留しなかった58.9%の答案では一致率80.9%でした。
日本語デモ36件は独自の回帰テストとして別集計。教材の推薦効果や学習効果は未検証です。

[追加の確認問題を使う模擬実験](benchmarks/simulation/RESULTS.md)も実施しました。
架空生徒90シナリオで、証拠不足を除いた5設定の順位は30.0%→36.7%。
確認問題の選択が全件同じになり、診断精度は十分には改善しませんでした。
